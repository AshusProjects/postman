...
def process_checkinout(emp_id):
    ...
    record = Attendance.query.filter_by(emp_id=emp_id, date=today).first()

    # 1 → First check-in
    if not record:
        record_checkin(emp_id, emp_name)
        pyttsx3.speak(f"{emp_name} checked in successfully")
        return f"{emp_name} checked in successfully!"

    # 2 → Already checked in → do check-out
    elif record.check_out is None:
        record_checkout(emp_id, emp_name)
        pyttsx3.speak(f"{emp_name} checked out successfully")
        return f"{emp_name} checked out successfully!"

    # 3 → Already checked out today
    else:
        pyttsx3.speak(f"{emp_name} has already checked out today")
        return f"{emp_name} has already checked out today!"
...
def record_checkin(emp_id, emp_name, official_start=time(9, 0), grace_minutes=30):
    ...
    record = Attendance.query.filter_by(emp_id=emp_id, date=today).first()
    if record:
        return f"already_checked_in|{record.check_in}"
...
def record_checkout(emp_id, emp_name, official_end=time(17, 0)):
    today = datetime.now().date()
    now_time = datetime.now().time().replace(microsecond=0)
    ...
    if not record:
        record = Attendance(emp_id=emp_id, emp_name=emp_name, date=today)

    if record.check_out:
        return f"already_checked_out|{record.check_out}"
...
def api_mark_attendance():
    ...
    if not record:
        ...
        return {
            "isSuccess": "true",
            "message": f"{emp_name} checked in successfully",
            "data": {
                "status": "check-in",
                "emp_id": emp_id,
                "name": emp_name
    ...
    if record.check_out is None:
        ...
        return {
            "isSuccess": "true",
            "message": f"{emp_name} checked out successfully",
            "data": {
                "status": "check-out",
                "emp_id": emp_id,
                "name": emp_name
    ...
    return {
        "isSuccess": "false",
        "message": f"{emp_name} has already checked out today"
    }, 409
...
def api_update_employee():
    ...
    admin = Employee.query.filter_by(emp_id=admin_id, Passward=admin_pass, role="admin").first()
    if not admin:
        return {"isSuccess": "false", "message": "Invalid admin login"}, 403
    ...
    if not employee:
        return {"isSuccess": "flse", "message": "Employee not found"}, 404
...
def video_feed():
    ...
    def generate():
        # FIX → Ensure generator uses Flask app context
        with app.app_context():
            ...
            if isinstance(result, tuple) and result[0] == "Recognized":
                ...
                if not emp:
                    ...
                    emp_name = emp.name
                    today = datetime.now().date()
                    record = Attendance.query.filter_by(emp_id=emp_id,
                                                        date=today).first()
                    ...
                    if not record:
                        ...
                        msg = f"{emp_name} has already checked out today!"
                        pyttsx3.speak(msg)

                # SEND MESSAGE FRAME
                frame = np.zeros((300, 600, 3), dtype=np.uint8)
                cv2.putText(frame, msg, (30, 150),
...
def new_employee():
    ...
    if request.method == "POST":
        ...
        # SOAP device confirmation
        device_status = "Successfully updated in device!" if "UpdateEmployeeResult" in soap_result else "Failed to update in device."

        return render_template("message.html",
                               message=f"Employee saved! {device_status}",
                               detail=soap_result,
                               buttons=[
...
def attendance():
    ...
    for emp in employees:
        ...
        attendance_data.append({
            ...
            "late_by": rec.late_by if rec else "—",
            "early_by": rec.early_by if rec else "—",
        })
